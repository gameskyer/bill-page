<template>
  <el-form ref="ruleFormRef" :inline="true" :model="ruleForm" class="demo-form-inline">
    ...
    <el-form-item label="类型" prop="values">
      <el-select v-model="ruleForm.values" placeholder="请选择类型(多选)" style="width: 240px" multiple clearable>
...
const total = ref();
SelectBill({ results: 100, page: 1 }).then((res) => {
...
const handleSizeChange = (val: number) => {
  SelectBill({ results: pageSize.value, page: currentPage.value }).then((res) => {
    tableData.value = res.data.bill;
  });
}
