<template>
  <el-form ref="ruleFormRef" :inline="true" :model="ruleForm" class="demo-form-inline">
    ...
    <el-form-item label="类型" prop="billTypes">
      <el-select v-model="ruleForm.billTypes" placeholder="请选择类型" style="width: 240px" collapse-tags clearable>
        <el-option v-for="option in options" :key="option.value" :label="option.label" :value="option.value" />
      </el-select>
    </el-form-item>
    <el-form-item>
      <el-button type="primary" :icon="Search" circle @click="select" />
      <el-button type="info" :icon="CircleCloseFilled" @click="resetForm(ruleFormRef)" circle />

      <el-button type="success" :icon="Check" @click="submitFrom" circle />
    </el-form-item>
  </el-form>
...
const submitFrom = async () => {
  ...
  updataParam.id = ids;
  updataParam.billType = ruleForm.billTypes[0];
